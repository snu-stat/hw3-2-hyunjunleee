...
완료. 이때 ENV PATH=/opt/conda/envs/r-reticulate/bin:${CONDA_DIR}/bin:${PATH} 를 넣고, ENV MPLBACKEND=Agg 를 넣으며, r 설치의 cran 설정을 추가하였다.
...
완료. 단 이때 render quarto단계를 다음과 같이 수정함
```{yml}
        run: |
          export CONDA_ENV=$(conda info --base)/envs/r-reticulate
          export RETICULATE_PYTHON=$CONDA_ENV/bin/python
          export LD_PRELOAD=$CONDA_ENV/lib/libexpat.so.1
...
완료. Binder 주소는 https://mybinder.org/v2/gh/snu-stat/hw3-2-hyunjunleee/gh-pages?filepath=hw03.ipynb 이다.